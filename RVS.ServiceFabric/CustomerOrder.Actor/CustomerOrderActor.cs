...
namespace CustomerOrder.Actor
{
    ...
    internal class CustomerOrderActor : Actor, IRVSSalesActor, IRemindable
    {
        private const string InventoryServiceName = "RVS.InventoryService";
        ...
        /// <returns></returns>
        public async Task SubmitOrderAsync(IEnumerable<RVSSalesActorItem> orderList)
        {
            try
            {
                await this.StateManager.SetStateAsync<List<RVSSalesActorItem>>(OrderItemListPropertyName, new List<RVSSalesActorItem>(orderList));
                await this.StateManager.SetStateAsync<RVSSalesActorStatus>(OrderStatusPropertyName, RVSSalesActorStatus.Submitted);
        ...
        internal async Task FulfillOrderAsync()
        {
            ...
            await this.SetOrderStatusAsync(RVSSalesActorStatus.InProcess);

            IList<RVSSalesActorItem> orderedItems = await this.StateManager.GetStateAsync<IList<RVSSalesActorItem>>(OrderItemListPropertyName);

            ActorEventSource.Current.ActorMessage(this, "Fullfilling customer order. ID: {0}. Items: {1}", this.Id.GetGuidId(), orderedItems.Count);

            foreach (RVSSalesActorItem tempitem in orderedItems)
            {
            ...
            foreach (RVSSalesActorItem item in orderedItems.Where(x => x.FulfillmentRemaining > 0))
            {
                ...
                {
                    await this.SetOrderStatusAsync(RVSSalesActorStatus.Canceled);
                    return;
                }
            ...
            IList<RVSSalesActorItem> items = await this.StateManager.GetStateAsync<IList<RVSSalesActorItem>>(OrderItemListPropertyName);
            bool backordered = false;

            // Set the status appropriately
            foreach (RVSSalesActorItem item in items)
            {
            ...
            if (backordered)
            {
                await this.SetOrderStatusAsync(RVSSalesActorStatus.Backordered);
            }
            else
            {
                await this.SetOrderStatusAsync(RVSSalesActorStatus.Shipped);
            }
